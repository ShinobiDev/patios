...
@section('header')
    <h1>
        Reporte Estadistico de Ingreso
    </h1>
    <small>Informe</small>

    <ol class="breadcrumb">
      <li><a href="{{route('home')}}"><i class="fa fa-dashboard">  Inicio</i></a></li>
      <li class="active">Reporte Estadistico Ingreso de Vehículo</li>
    </0l>
...
@section('content')
  ...
  <div class="box box-primary">
    <div class="box-header">
        <h3 class="box-title">Reporte Estadistico Ingreso de Vehículo</h3>
        ...
        <table id="ingreso-table" class="table table-bordered table-striped">
          ...
          <tbody>
            @foreach ($vista as $vistas)
              <tr>
                <th>{{$vistas->codigo}}</th>
                <th>{{$vistas->cantidad}}</th>
                <th>{{$vistas->cantidad/30}}</th>
              </tr>
            @endforeach
          </tbody>
        </table>
      ...
      $('#ingreso-table').DataTable( {
...

          {
                  ...
                  action: function ( e, dt, node, config ) {
                      txt = document.getElementById("ingreso-table");
